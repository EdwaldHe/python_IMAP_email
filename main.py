...
Extract selected mails from your other email account:
only need email address and normal password
...
if __name__ == '__main__':
    ...
    # key = 'FROM'
    # value = 'edwald.he@gmail.com'
    # # Search for emails with specific key and value
    # _, data = my_mail.search(None, key, value)

    # get recent 2 days emails
    date = (datetime.date.today() -
            datetime.timedelta(days=2)).strftime("%d-%b-%Y")
    _, data = my_mail.search(None, '(ALL)', f'(SENTSINCE {date})')
    ...
    for msg in msgs[::-1]:
        for response_part in msg:
            if type(response_part) is tuple:
                ...
                print("==========Reading Email Start===================================")
                print("Subject:", my_msg['subject'])
                print("From:", my_msg['from'])
                print("Sent On:", my_msg['Date'])
                print("Body:")
